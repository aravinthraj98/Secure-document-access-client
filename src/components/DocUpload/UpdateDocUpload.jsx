...
function UpdateDocUpload({ data,id,setUpdate }) {
  let initialState = {
    fileValue: '',
    docName: '',
    fileBuffer: '',
  ...
  useEffect(() => {
    if (data) {
      console.log({data})
      loadDepartments(data.data);
    }
  }, [data]);

  const loadDepartments = async (data) => {
    ...
    if (departments != null) {
      for (let i in data.status) {
        ...
        if (data.status[i][3] == true) {
          let index = tempDept.indexOf(data.status[i][0]);
          tempDept.splice(index, 1);
        }
      }
      for(let i in data.approvalDept){
        if(data.approvalDept[i][0]!=''){
           let index = tempDept.indexOf(data.approvalDept[i][0]);
          tempDept.splice(index, 1);
        }
      }
    }
    ...
    for (let i in data.src) {
      let temp = {
        docName: data.src[i][1],
        url: data.src[i][0],
      };
      file_temp.push(temp);
    }
    setExistingFiles(file_temp);
  ...
  return (
    <>
      ...
      <div
        className=' container-fluid w-100 bgcolor'
        style={{ height: '100vh' }}
      >
        <div>-</div>
        <form
        ...
        >
          ...
          <div className='row m-5'>
            <div className='col-md-4'>Upload document</div>
            <div className='col-md-8'>
              <input
                type='file'
                id='file'
              ...
              <div className='row'>
                {files.map((value, index) => (
                  ...
                  >
                    ...
                    >
                      {' '}
                      <div>{value.docName}</div>
                      <div
                        onClick={() => setModalData(value.url)}
                        style={{
                          color: 'lightblue',
                          fontWeight: 'bold',
                          margin: 2,
          ...
          <div className='row m-3'>
            {departments.map((value, index) => (
              <div className='col-md-2 text-info font-weight-bold border-box'>
                <input
                  type='checkbox'
                  value={value}
                  onClick={handleCheck}
          ...
          <div className='row'>
            <div className='col-md-6'>
              ...
              <textarea
                onChange={(e) =>
                  setFile({ ...file, fullAddress: e.target.value })
                }
                rows={3}
            ...
            <div className='col-md-6'>
              Enter description about your request:
              <textarea
                onChange={(e) =>
...
export default UpdateDocUpload;
